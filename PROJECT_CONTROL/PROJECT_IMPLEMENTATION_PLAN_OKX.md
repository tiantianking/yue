...
已完成：阶段 0 项目控制、阶段 1 基础工程。

下一步：实现 OKX 历史数据读取、质量审计、已闭合 K 线过滤。

说明：下面是原报告全文实施版，已把交易所口径统一改成 OKX；原报告中的代码块、表格和阶段拆分保留。

---
# OKX 轻量化加密货币合约回测与半自动信号系统项目详细实施表
...
| 交易所 | OKX / 抽象适配层 | **固定 OKX，内部保留适配层边界** | MVP 不下单，先把回测和信号引擎做对；后续只落地 OKX 只读 / dry-run / 人工确认适配器。 |
| 合约类型 | USDT 线性永续 / 反向永续 | **USDT 线性永续** | 线性合约的仓位、PnL、止损和爆仓近似更容易统一。 |
| 手续费假设 | maker / taker 分开；全部按 taker | **全部按 taker** | 为保守起见，回测默认把入场和出场都按 taker 算。OKX 费率以后按账户实际档位接入；MVP 先用保守 taker 估算。citeturn20view2 |
...
| 未来交易所对接 | `OKX 原生 API 或 OKX 原生 API 或 CCXT 只读层 只读层` 仅用于后续 dry-run/只读适配 | 原生 REST SDK | OKX 原生 API 或 OKX 原生 API 或 CCXT 只读层 只读层 提供统一 API 和多交易所支持，但 MVP 不启用自动下单。citeturn14search1turn20view3 |
...
内部建议统一使用 **UTC** 作为引擎时区，界面再按你的本地时区展示。原因不是“习惯问题”，而是资金费、结算和很多交易所文档都以 UTC 时间组织；OKX 的资金费说明都直接给出 UTC 时点。citeturn12view3turn7search4
...
在未指定交易所前，建议采取“**交易所无关的保守近似 + 交易所适配器可替换**”方案。OKX 实施版使用交易所无关的逐仓保守近似，并在后续接入 OKX 维护保证金档位后校准：
...
并说明 isolated margin 下，单个仓位的最大损失受限于该仓位的保证金；同时资金费若从仓位保证金中扣减，会把爆仓价推近 mark price。OKX 官方文档也说明维护保证金取决于 notional 所处的风险梯度。citeturn20view0turn12view4turn12view3turn20view1
...
另外，OKX 实施版也必须把手续费缓冲纳入保证金风险估算；这意味着做保守模拟时，**手续费缓冲本来就是保证金风险的一部分**，不该只算到账后 PnL。citeturn20view2
...
| 成本层 | 默认处理 | 备注 |
|---|---|---|
| 手续费 | 入场、出场均按 taker 计 | 默认 `0.06%`/side，OKX 实施版保守默认，后续按 OKX 实际费率档位替换 |
| 滑点 | 按成交方向加减 bps | 常规 5 bps，压力 10 bps |
| 资金费率 | 只有跨过 funding 时点才计入 | 基准 `0.01%/8h`，压力用 `0.03%/8h` 与 `0.01%/4h` |

OKX 实施版按 OKX fundingTime / nextFundingTime 判断是否跨过资金费时点；若在该时点之前已平仓，则不收资金费。资金费频率不得写死为 8 小时，必须允许 OKX 动态频率。citeturn12view3turn7search4turn7search12
...
OKX 实施版默认使用单向持仓口径，单币同一时间只允许一个方向仓位，这也很适合你用来简化单币级回测逻辑。citeturn20view2
...
这一步不要做成“点击确认就直接下单”。即便以后接交易所 API，也应该先做 **dry-run / order preview**，再让你确认。OKX 支持 market、limit、条件单、TP/SL 等订单能力；但本项目 MVP 不自动下单，只保留 OKX dry-run / 订单预览，非常适合将来补一个“只读行情 + 订单预览”的适配层；但这必须是**后续迭代**，不能进 MVP。citeturn20view2turn14search1turn20view3
...
| Phase 2 | 接入交易所OKX 真实 funding history / mark price / maintenance margin tier | 确定交易所后 |
...
| Phase 3 | OKX 只读适配、订单预览 | 需要把手工操作再缩短一步 |
