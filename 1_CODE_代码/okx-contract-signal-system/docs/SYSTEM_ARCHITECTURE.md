...
SIGNAL_ONLY 回测不依赖交易所下单数量。`validate_signal` 接受信号但不返回 `qty` 时，回测使用本地研究风险金额生成标准化结果行，并输出 `outcome`、`net_r` 和 `final_net_r`。参数搜索、每日学习、滚动回测、启动质量门和报告写入必须先校验回测结果；空结果或缺少核心列时只能失败或跳过，不能生成可信训练结论或正式报告。
