...
GUI、实时监控、调度器、报告任务和 TradingBrain 观察路径统一复用 `SignalScanService`。该服务负责闭合 K 线检查、最新 K 线边界、投票门、质量模型旁路、候选排序、A/B/C 分层和生命周期写入。只有 A 级正式候选会进入正式飞书信号推送和生命周期记录；B 级保留摘要，C 级只作为观察候选，不触发正式信号状态。
