...
> **新会话永久提醒：**过拟合与未来函数为零容忍硬门槛。发现风险或无法排除风险即停止；封存只开一次，失败不得营救，通过后仍需真实前向影子。H22、V357只能依靠真实前向证据完成最终确认。
...
新候选在打开任何PnL前必须同时提交并通过：

- 6—10个月历史封存段，默认8个月；起止时间必须位于完整历史数据范围内；
- 封存切分清单、数据快照SHA256和切分SHA256；封存访问次数必须为0；
- 完整策略家族试验账本，改币、改方向、改窗口、改持有期、加过滤器、加覆盖层和改名重测都必须登记；
- 点时字段证据，每个输入字段必须写明实际可用时间规则与修订策略；
- 完整代码依赖清单，候选引用的全部策略和辅助文件必须进入自动扫描；
- 最大持有期、标签观察期、purge和embargo；`purge`不得短于完整标签观察期；
- 上述证据文件必须存在，声明SHA256必须与文件实际哈希一致。

自动AST扫描还会阻断：显式或变量形式的负向位移、居中滚动、`bfill/backfill`、向前或最近`merge_asof`、向后插值、部分向前`iloc`访问及未来标签字段。自动扫描通过只是必要条件，不代表可以省略点时和逐信号时间戳审计。

任一证据缺失、哈希不一致、封存被打开或purge不足，候选立即失败，不允许进入收益验证。
