...
第四条冻结轨道 `momentum_staggered_3x3_refresh` 将资金等分为3个子组合，分别错开0、1、2天，每个子组合仍只每3天刷新一次。账户聚合目标每天只更新其中一个子组合，因此按日记录前向观察；至少60天且50次闭合观察才进行阶段判断，优先在90天且80次闭合观察时进入人工复核或失败归档。该轨道只是同一14日动量的执行分散版本，不能替代固定3日轨道，也不能据此把历史结果改判为A级。
