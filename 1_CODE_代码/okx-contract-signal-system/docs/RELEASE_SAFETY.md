...
- Package metadata, launcher display, and GUI display are synchronized to `3.56.34`; approved strategy identity remains `3.56.15`, and strict research identity remains `v3.56-strict`.
...
- `config/research_universe_policy.json` defines the operator-selected 21 mature swaps as the maximum allowed trading pool, not mandatory strategy breadth. New candidates may prospectively freeze any mechanism- or data-driven subset from one to 21 symbols before PnL; outcome-driven symbol selection and post-hoc deletion remain forbidden. H22 and V357 keep their original frozen member rules, and H27 remains record-only.
