...
from okx_signal_system.risk.costs import CostConfig, estimate_costs, participation_rate, slippage_bps_for_participation
...
def label_signal(signal: TradeSignal, future_bars: pd.DataFrame) -> SignalLabel | None:
    ...
    slippage_bps = _slippage_bps_for_row(window.iloc[0], entry_ref)
...
def _slippage_bps_for_row(row: pd.Series, entry_price: float) -> float:
    ...
    try:
        ...
        return slippage_bps_for_participation(rate, base_bps=CostConfig().normal_slippage_bps)
    except Exception:
        return CostConfig().normal_slippage_bps
